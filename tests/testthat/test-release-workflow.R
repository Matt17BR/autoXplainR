test_that("one call retains evidence and writes a report without recomputation", {
  ...
  expect_identical(as.character(render_model_report(result, second)), normalizePath(second))
...
test_that("fitting only and input errors are explicit", {
  expect_null(autoxplain(model_set = "quick", mtcars, "mpg", explain = FALSE)$explanations)
  expect_error(autoxplain(model_set = "quick", mtcars, "mpg", explain = NA), "explain")
  expect_error(autoxplain(model_set = "quick", mtcars, "mpg", enable_preprocessing = 1), "enable_preprocessing")
  expect_error(autoxplain(model_set = "quick", mtcars, "mpg", report = "bad.pdf"), "html")
})
...
test_that("raw predictions use the saved recipe and survive serialization", {
  ...
  result <- autoxplain(model_set = "quick", train, "y", explain = FALSE)
...
test_that("classification prediction preserves levels including a one-row matrix", {
  ...
  result <- autoxplain(model_set = "quick", binary, "am", explain = FALSE)
...
test_that("evaluation may omit trained classes without inventing metrics", {
  ...
  binary$Species <- NULL
  result <- autoxplain(model_set = "quick", binary, "y", test_data = binary[1:10, ], overlap_action = "ignore")
  ...
  multi <- autoxplain(model_set = "quick", iris, "Species", test_data = iris[1:10, ], overlap_action = "ignore")
