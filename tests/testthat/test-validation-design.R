...
test_that("group holdouts and tuning keep entire units separate", {
  ...
  expect_true(all(vapply(split(plan$fold_ids, plan$training_groups), function(x) length(unique(x)) == 1L,
                         logical(1))))
...
test_that("temporal boundaries preserve ties, order and gap", {
  ...
  result <- autoxplain(data, "y", validation = validation_split(time = "time", gap = 2), explain = FALSE)
  plan <- result$validation
  ...
  expect_error(autoxplain(data, "y", validation = validation_split(time = "time"), model_set = "tuned"),
               "rolling-origin")
})
...
test_that("validation specifications fail before training when infeasible", {
  ...
  expect_error(autoxplain(data, "y", validation = list()), "validation_split")
  expect_error(autoxplain(data, "y", validation = spec, engine = "h2o"), "base engine")
  expect_error(autoxplain(data, "y", validation = spec, test_data = data), "not both")
  expect_error(autoxplain(data, "y", validation = validation_split(group = "y")), "differ")
  expect_error(autoxplain(data, "y", validation = spec, model_set = "tuned", portfolio = "core",
                          tuning_control = tuning_control(fold_ids = rep(1:3, 40))), "own fold IDs")
  data$site[1] <- NA
  expect_error(autoxplain(data, "y", validation = spec), "without missing")
  data$site <- 1
  expect_error(autoxplain(data, "y", validation = spec), "at least three")
  expect_error(autoxplain(data, "y", validation = validation_split(time = "time", gap = 100)), "Too few")
})
...
test_that("outer test labels cannot change grouped tuning or preprocessing", {
  ...
  data$time <- NULL
  fit <- function(x) {
    autoxplain(x, "y", validation = validation_split(group = "site"),
  ...
  expect_identical(first$preprocessing_metadata$training_data$recipe,
