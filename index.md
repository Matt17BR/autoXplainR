...
**[Roadmap](https://matt17br.github.io/autoXplainR/ROADMAP.md)**
...
regression and binary outcomes), tree, random-forest, and
gradient-boosted-tree families. `portfolio = "extended"` adds neural
networks, radial SVMs, weighted nearest neighbors, and MARS.
`portfolio = "core"` is the dependency-light linear/tree/neural
tournament. Missing engines produce one explicit installation
instruction; AutoXplainR never changes the tournament according to
...
Behavior cards describe known representational capacity; held-out
performance, same-row prediction gaps, and optional
permutation-importance differences are computed evidence. Keeping those
labels separate prevents “this model *can* learn interactions” from
becoming the false claim that it *did* use an interaction in this fit.
H2O AutoML remains optional when a Java-backed search or stacked
...
AutoXplainR shows held-out performance, fit and prediction time, model
size, complexity, and the candidate-relative Pareto frontier.
...
_(Performance versus model size, with the interpretation rule and full candidate table kept in the frame.)
...
The repository includes [complete verified narrative
snapshots](https://matt17br.github.io/autoXplainR/LLM_EXAMPLES.md) from
one reproducible tuned `iris` analysis: the deterministic local memo and
a live `gemini-3.5-flash` response generated on 2026-07-16. The Gemini
call used validated structured output, `store = false`, and no fallback.

Here is a short excerpt from that Gemini response:

> **Held-out performance.** On the held-out test set, the reference
> tuned neural network achieved a final log loss of 0.08206, a 92.5%
> relative improvement over the simple baseline. This final held-out
> performance is distinct from the training-resampled selection score of
> 0.09566, calculated with five training-only folds.
...
[governance](https://matt17br.github.io/autoXplainR/GOVERNANCE.md),
[release
