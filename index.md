...
explicit mode automatically tunes statistical, tree, and neural-network
models without peeking at the final holdout; another compares
approachable candidates and Pareto trade-offs. Advanced users can bring
any fitted model, use H2O AutoML, or opt into local or hosted language
models.
...
AutoXplainR searches across a statistical reference, decision-tree depth
and pruning settings, and scaled neural-network size and weight decay.
Preprocessing is relearned inside each resampling fold. The default
one-standard-error rule chooses a simpler configuration when its
training-resampled error is hard to distinguish from the best observed
error.
...
The complete candidate grid, fold-level scores, warnings, failures,
elapsed times, selected hyperparameters, and data boundary remain
available in `tuning_results(tuned)`. H2O AutoML is still supported when
a much broader, Java-backed search is appropriate.
