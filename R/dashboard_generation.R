...
prepare_dashboard_data <- function(autoxplain_result, top_features, sample_instances) {
  ...
  # Detect task type for PDP calculation
  is_classification <- !is.numeric(data[[target_column]])
  ...
  for (i in seq_along(models_to_analyze)) {
    ...
    tryCatch({
      ...
      pdp_list[[model_name]] <- calculate_partial_dependence_multi(
        ...
        return_all_classes = is_classification
      )
    }, error = function(e) {
...
create_dashboard_rmd <- function(data_files) {
  ...
  rmd_content <- c(
    ...
    "        }",
    "      } else {",
    "        # For classification: prioritize appropriate classification metrics",
    "        selected_metric <- if ('AUC' %in% metric_names) {",
    "          'AUC'",
    "        } else if ('auc' %in% metric_names) {",
    "          'auc'",
    "        } else if ('logloss' %in% metric_names) {",
    "          'logloss'",
    "        } else if ('mean_per_class_error' %in% metric_names) {",
    "          'mean_per_class_error'",
    "        } else {",
    "          # Filter out timestamp-like metrics and use first actual performance metric",
    "          valid_metrics <- metric_names[!grepl('timestamp|time|date', metric_names, ignore.case = TRUE)]",
    "          if (length(valid_metrics) > 0) valid_metrics[1] else metric_names[1]",
    "        }",
    "      }",
    "      ",
    ...
    "    'logloss' = 'Log Loss',",
    "    'mean_per_class_error' = 'Classification Error Rate',",
    "    'rmse' = 'RMSE (Root Mean Squared Error)',",
    "    'mae' = 'MAE (Mean Absolute Error)',",
    "    'mse' = 'MSE (Mean Squared Error)',",
    "    'scoring_time' = 'Scoring Time (seconds)',",
    ...
    "    gsub('_', ' ', tools::toTitleCase(tolower(metric_name)))",
    "  )",
    "  ",
  ...
  rmd_content <- c(rmd_content,
    ...
    "        } else {",
    "          list()  # No 'Top 3 Models' option for classification",
    "        },",
    "        lapply(seq_along(models_analyzed), function(i) {",
    "          if (task_type == 'Classification' && i == 1) {",
    ...
    "    )",
    "  ),",
    "  htmltools::tags$script(htmltools::HTML('function updatePDPPlots() { var selector = document.getElementById(\"model-selector-pdp\"); var selectedModel = selector.value; var allModelsContainer = document.getElementById(\"pdp-all-models\"); var allIndividualContainers = document.querySelectorAll(\"[id^=pdp-model-]\"); if (selectedModel === \"ALL_MODELS\") { if (allModelsContainer) allModelsContainer.style.display = \"block\"; for (var i = 0; i < allIndividualContainers.length; i++) { allIndividualContainers[i].style.display = \"none\"; } } else { if (allModelsContainer) allModelsContainer.style.display = \"none\"; for (var i = 0; i < allIndividualContainers.length; i++) { allIndividualContainers[i].style.display = \"none\"; } var cleanModelName = selectedModel.replace(/[^a-zA-Z0-9]/g, \"-\"); var targetContainer = document.getElementById(\"pdp-model-\" + cleanModelName); if (targetContainer) { targetContainer.style.display = \"block\"; } } } window.addEventListener(\"DOMContentLoaded\", function() { updatePDPPlots(); });'))",
    ")",
    ...
    "  }",
    "  ",
    "  # Create container for all models view with 2x2 grid (regression only)",
    "  if (task_type == 'Regression') {",
    ...
    "    )",
    "  } else {",
    "    all_models_container <- NULL  # No combined view for classification",
    "  }",
    "}",
    "",
    ...
    "      ",
    "      if (!is.null(data) && nrow(data) > 1) {",
    "        # Use the updated plot_partial_dependence function which handles multi-class",
    "        p <- plot_partial_dependence(data)",
    "        ",
    ...
    "      prob_cols <- grep('^prob_', colnames(data), value = TRUE)",
    "      is_multiclass <- length(prob_cols) > 0",
    "      ",
    "      if (is_multiclass) {",
    "        # Multi-class case: analyze each class probability",
    "        class_insights <- character()",
    "        for (prob_col in prob_cols) {",
    "          class_name <- gsub('^prob_', '', prob_col)",
    "          y_values <- data[[prob_col]]",
    "          y_range <- max(y_values) - min(y_values)",
    "          direction <- ifelse(y_values[length(y_values)] > y_values[1], 'increase', 'decrease')",
    "          magnitude <- round(y_range, 3)",
    "          ",
    "          if (y_range > 0.05) {  # More lenient threshold for probabilities (0-1 scale)",
    "            class_insights <- c(class_insights, paste0(",
    "              '<span style=\"color: #1976d2;\">P(', class_name, ')</span> shows <strong>', ", 
    "              direction, '</strong> by ~', magnitude, ' (', round(y_range * 100, 1), '%)'))",
    "          }",
    "        }",
    "        ",
    "        if (length(class_insights) > 0) {",
    "          # Determine overall impact color based on maximum range across classes",
    "          max_range <- max(sapply(prob_cols, function(col) max(data[[col]]) - min(data[[col]])))",
    "          if (max_range > 0.3) {",
    "            color <- '#d32f2f'  # High impact",
    "          } else if (max_range > 0.1) {",
    "            color <- '#f57c00'  # Moderate impact",
    "          } else {",
    "            color <- '#388e3c'  # Low impact",
    "          }",
    "          ",
    "          feature_type <- ifelse(feature_name %in% names(feature_types), feature_types[[feature_name]], 'numerical')",
    "          explanation <- paste(class_insights, collapse = '<br>')",
    "          ",
    "          insights_html <- paste0(insights_html, ",
    ...
    "          impact_desc <- 'DETECTABLE INFLUENCE'",
    "          recommendation <- 'Measurable impact'",
    "          color <- '#1976d2'",
    "        } else {",
    ...
    "  if (insights_html == '<div style=\"font-family: Arial, sans-serif; line-height: 1.4; padding: 15px; background-color: #f8f9fa; border-radius: 8px; border-left: 4px solid #2196f3;\"><h4 style=\"color: #1976d2; margin-top: 0;\">🎯 Key Insights</h4>') {",
    "    # No insights were generated, add general ones",
    "    insights_html <- paste0(insights_html,",
    "      '<div style=\"margin: 8px 0; padding: 8px; background: white; border-radius: 4px; border-left: 3px solid #2196f3;\">',",
    "      '<strong style=\"color: #1976d2;\">📊 Feature Analysis:</strong><br>',",
    "      '<span style=\"color: #333;\">PDP analysis was conducted for ', n_features, ' features. While individual feature effects may be subtle, the combined feature set enables effective model predictions.</span>',",
    "      '</div>',",
    "      '<div style=\"margin: 8px 0; padding: 8px; background: white; border-radius: 4px; border-left: 3px solid #4caf50;\">',",
    "      '<strong style=\"color: #4caf50;\">🎯 Model Performance:</strong><br>',",
    "      '<span style=\"color: #333;\">The trained models successfully learned patterns from the data, with the best model selected based on validation performance.</span>',",
    "      '</div>')",
    "  }",
    "  ",
    ...
    "} else {",
    "  # Generate fallback key insights when PDP data is not available",
    "  fallback_insights_html <- '<div style=\"font-family: Arial, sans-serif; line-height: 1.4; padding: 15px; background-color: #f8f9fa; border-radius: 8px; border-left: 4px solid #2196f3;\">'",
    "  fallback_insights_html <- paste0(fallback_insights_html, '<h4 style=\"color: #1976d2; margin-top: 0;\">🎯 Key Insights</h4>')",
    "  ",
    "  # Extract insights from importance data if available",
    paste0("  importance_data <- readRDS('", data_files$importance, "')"),
    "  if (!is.null(importance_data) && nrow(importance_data) > 0) {",
    "    # Get top features from importance",
    "    top_features <- head(importance_data[order(-importance_data$importance), ], 3)",
    "    ",
    "    fallback_insights_html <- paste0(fallback_insights_html, ",
    "      '<div style=\"margin: 8px 0; padding: 8px; background: white; border-radius: 4px; border-left: 3px solid #2196f3;\">',",
    "      '<strong style=\"color: #1976d2;\">📈 Most Influential Features:</strong><br>',",
    "      '<span style=\"color: #333;\">The top predictive features are <strong>', ",
    "      paste(top_features$feature[1:min(3, nrow(top_features))], collapse = '</strong>, <strong>'), ",
    "      '</strong>, indicating these variables have the strongest impact on model predictions.</span>',",
    "      '</div>')",
    "  }",
    "  ",
    "  # Add model characteristics insights",
    paste0("  model_characteristics <- readRDS('", data_files$model_characteristics, "')"),
    "  if (!is.null(model_characteristics) && length(model_characteristics) > 0) {",
    "    best_model <- model_characteristics[[1]]",
    "    fallback_insights_html <- paste0(fallback_insights_html,",
    "      '<div style=\"margin: 8px 0; padding: 8px; background: white; border-radius: 4px; border-left: 3px solid #4caf50;\">',",
    "      '<strong style=\"color: #4caf50;\">🏆 Best Performing Model:</strong><br>',",
    "      '<span style=\"color: #333;\">The <strong>', best_model$algorithm, '</strong> algorithm achieved the best performance and should be your primary choice for predictions.</span>',",
    "      '</div>')",
    "    ",
    "    # Add diversity insight if multiple models",
    "    if (length(model_characteristics) > 1) {",
    "      algorithms <- unique(sapply(model_characteristics, function(x) x$algorithm))",
    "      fallback_insights_html <- paste0(fallback_insights_html,",
    "        '<div style=\"margin: 8px 0; padding: 8px; background: white; border-radius: 4px; border-left: 3px solid #ff9800;\">',",
    "        '<strong style=\"color: #ff9800;\">🔄 Algorithm Diversity:</strong><br>',",
    "        '<span style=\"color: #333;\">Tested <strong>', length(algorithms), '</strong> different algorithm types (',",
    "        paste(algorithms, collapse = ', '), ') to ensure robust model selection.</span>',",
    "        '</div>')",
    "    }",
    "  }",
    "  ",
    "  # Add general AutoML insight",
    "  fallback_insights_html <- paste0(fallback_insights_html,",
    "    '<div style=\"margin: 8px 0; padding: 8px; background: white; border-radius: 4px; border-left: 3px solid #9c27b0;\">',",
    "    '<strong style=\"color: #9c27b0;\">🤖 AutoML Advantage:</strong><br>',",
    "    '<span style=\"color: #333;\">Automated machine learning systematically evaluated multiple approaches, saving time while ensuring you get the best possible model for your data.</span>',",
    "    '</div>')",
    "  ",
    "  fallback_insights_html <- paste0(fallback_insights_html, '</div>')",
    "  htmltools::HTML(fallback_insights_html)",
    "}",
    "```",
    "",
  ...
  rmd_content <- c(rmd_content,
    ...
    "    intersect(available_metrics, c('AUC', 'auc', 'accuracy', 'logloss', 'mean_per_class_error'))",
    "  } else {",
    "    intersect(available_metrics, c('RMSE', 'rmse', 'mae', 'MAE', 'r2', 'R2'))",
    "  }",
    "  additional_metrics <- if (is_classification) {",
    "    # For classification, include useful additional metrics that are commonly available",
    "    intersect(available_metrics, c('r2', 'R2', 'precision', 'recall', 'f1', 'MSE', 'RMSE'))",
    "  } else {",
    "    intersect(available_metrics, c('AIC', 'aic'))",
    "  }",
    "  ",
    "  # Combine metrics: up to 3 performance + 2 additional for comprehensive display",
    "  selected_metrics <- c(head(primary_metrics, 3), head(additional_metrics, 2))",
    ...
    "  is_classification <- !is.numeric(autoxplain_result$training_data[[autoxplain_result$target_column]])",
    "  ",
    "  # Get available metrics from first model to determine best primary metric",
    "  available_metrics <- if (!is.null(model_characteristics[[1]]$performance_metrics)) {",
    "    names(model_characteristics[[1]]$performance_metrics)",
    "  } else {",
    "    character(0)",
    "  }",
    "  ",
    "  # Select primary metric using same logic as performance plot",
    "  primary_metric <- if (is_classification) {",
    "    if ('AUC' %in% available_metrics) {",
    "      'AUC'",
    "    } else if ('auc' %in% available_metrics) {",
    "      'auc'",
    ...
    "  lower_is_better <- grepl('loss|error|mse|rmse|mae', tolower(primary_metric))",
    "  metric_direction <- if (lower_is_better) 'asc' else 'desc'",
    "  ",
    "  for (i in seq_along(model_characteristics)) {",
    "    model <- model_characteristics[[i]]",
    "    ",
    ...
    "        # Store primary metric for ranking",
    "        if (tolower(metric_name) == tolower(primary_metric)) {",
    "          perf_score <- value",
    ...
    "          perf_score <- value",
    "        }",
    "        # Enhanced formatting - highlight the primary metric",
    "        is_primary <- tolower(metric_name) == tolower(primary_metric)",
    "        ",
