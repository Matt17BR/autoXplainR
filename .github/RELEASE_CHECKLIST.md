...
- Check that same source archive with current R-devel. The release workflow
  omits only the already-validated manuals in this second check, so the exact
  R-release-built archive still exercises R-devel code, examples, tests, and
  vignettes. Explain every remaining note in `cran-comments.md`.
