...
1.  **Existing-model adapters.** Document tested prediction contracts
    for common tidymodels/DALEX/mlr3 workflows, including preprocessing,
    probability-column order and raw-row prediction. Prefer adapters
    over additional learner families.
...
4.  **Compatibility and performance.** Publish result-schema migration
    rules and aggregate JSON fixtures. Measure prediction-call counts,
    runtime and memory on wide/tall data before adding batching or
    parallel execution.
