...
## Implemented repairs and acceptance checkpoints

The first integrated gates below passed on the refreshed implementation on
2026-09-07. The extended-portfolio task then exposed two interface defects, and
its fresh-process installed-archive replay exposed a saved-model failure. Those
were repaired and the same task was repeated without refitting. Counts in this
section preserve the earlier checkpoint; the final revision, expanded gates and
artifact status belong in the [release evidence](release-0.6.0.md). The concrete
reader answers, counterexamples and repeated tasks are the acceptance evidence.
...
| Challenge an extended portfolio from an installed package. | Eleven retained models remain inspectable. The GAM's native basis dimensions are 5 per smooth; `k = 5` has CV RMSE 0.649747, inside the independently calculated 0.683299 limit despite `k = 8` having lower loss. Optional-model effects, the source of XGBoost's largest error and three copied R calls match native predictions. The repaired result also works in a fresh R process. | [Extended-portfolio walkthrough](results/extended-walkthrough-0.6.0.md), including the exact local archive digest, original failing states and successful installed replay. |
...
The initial integrated main browser gate passed 1,279 checks across regression, binary,
multiclass and quick workflows. At that checkpoint the independent hand-calculated
chart fixture passed 63 checks; it is also invoked as one aggregate main-gate check and should
...
The extended task showed why those passing counts were insufficient. A GAM's
compact identity lost its effective `k` value even though Selection had the
requested limit; it now exposes native per-input basis dimensions separately
from learned EDF. Eleven cost labels overlapped where four-model fixtures had
looked readable; dense charts now use separate label positions and connecting
lines, with clustered, tied-cost and no-JavaScript cases in the chart gate.
Finally, a saved neural model failed in a new R process because its native S3
prediction method had not been loaded. Explicit core namespace imports and a
separate-process installed reload guard repair that path. The extended task was
rerun from the repaired archive, preserving the original data, fits and answers.
...
[AutoXplainR 0.6.0](https://github.com/Matt17BR/autoXplainR/releases/tag/v0.6.0)
was published after the supported-platform, current/minimum-engine, browser,
quality and exact-archive gates passed. The actual GitHub download was installed
into a new library and passed the parent and separate-session saved-model
workflows. Replaying the original eleven-model analysis through that download
produced the identical inspected HTML and preserved the original RDS bytes.
All three live reports and eight screenshots match the reviewed repository
files; the online walkthrough passed at desktop and phone widths.

The [release record](release-0.6.0.md) gives the tag, exact checksums, CI links,
observed limitations and final acceptance evidence. The
[portable extended walkthrough](extended-walkthrough/README.md) preserves the
data generator, browser tasks and independent native checks for another review.
Its guide requires inspecting the answers and charts before reading the oracles;
a script's exit code alone is not the usability standard.
